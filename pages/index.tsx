...
function calcPolitica(o: string): string {
  const m: any = {
...
function exportToExcel(data: Credencial[]) {
  const headers = ['Credencial','Nome','Cargo','Especificação','Empresa','Abrangência','Placa(s)','Credencial Física','Política','Seção','Setor','Departamento','Área','Divisão','Unidade','Núcleo','Grupo','Status']
  const rows = data.map(r => [r.crede,r.usuario,r.ocupacao,r.especificacao,r.empresa,r.abrangencia,r.placa,r.credencial_fisica,r.politica,r.secao,r.setor,r.departamento,r.area,r.divisao,r.unidade,r.nucleo,r.grupo,r.reserva==='SIM'?'RESERVA':'EM USO'])
  const bom = '\uFEFF'
...
export default function Home() {
  ...
  function openEdit(r: Credencial) { setForm({...r}); setEditId(r.id ?? null); setErro(''); setModal(true) }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-screen-xl mx-auto px-6 py-6">
        <div className="flex gap-3 mb-5 flex-wrap">
          {[{label:'Total',val:credenciais.length,color:'bg-blue-50 text-blue-700'},{label:'Em uso',val:emUso,color:'bg-green-50 text-green-700'},{label:'Reserva',val:reserva,color:'bg-gray-100 text-gray-600'}].map(s=>(
          ...
          <div className="ml-auto flex gap-2">
            <button onClick={()=>exportToExcel(filtered)} className="bg-green-600 hover:bg-green-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition flex items-center gap-2">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/>
              </svg>
              Exportar Excel
            </button>
            <button onClick={openNew} className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition">+ Nova credencial</button>
          </div>
        </div>

        <div className="bg-white rounded-xl border border-gray-200 p-3 mb-4 flex gap-2 flex-wrap">
          <input value={busca} onChange={e=>setBusca(e.target.value)} placeholder="Buscar nome, credencial, empresa, placa..." className="flex-1 min-w-48 text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>{['Credencial','Nome','Cargo','Especificação','Empresa','Placa(s)','Política','Ações'].map(h=><th key={h} className="text-left px-4 py-3 text-xs font-medium text-gray-500 whitespace-nowrap">{h}</th>)}</tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {filtered.length===0&&<tr><td colSpan={8} className="text-center py-12 text-gray-400">Nenhum resultado encontrado.</td></tr>}
                  {filtered.map(r=>(
                    <tr key={r.id} className="hover:bg-gray-50 transition">
                      <td className="px-4 py-3 font-semibold text-gray-900">{r.crede}</td>
                      <td className="px-4 py-3 max-w-[200px]">{r.reserva==='SIM'?<span className="bg-gray-100 text-gray-500 text-xs px-2 py-0.5 rounded-md">RESERVA</span>:<span className="truncate block">{r.usuario||'—'}</span>}</td>
                      ...
                      <td className="px-4 py-3">{r.politica?<span className={`text-xs font-medium px-2 py-0.5 rounded-md ${polColor(r.politica)}`}>{r.politica}</span>:<span className="text-gray-300 text-xs">—</span>}</td>
                      <td className="px-4 py-3"><div className="flex gap-2"><button onClick={()=>openEdit(r)} className="text-xs text-blue-600 hover:underline">Editar</button><button onClick={()=>r.id&&excluir(r.id)} className="text-xs text-red-500 hover:underline">Excluir</button></div></td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <form onSubmit={salvar} className="px-6 py-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={form.crede} onChange={e=>handleFormChange('crede',e.target.value)} disabled={!!editId} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-50" placeholder="Ex: 7980"/>
                </div>
                <div>
              ...
              <div>
                ...
                <input value={form.usuario} onChange={e=>handleFormChange('usuario',e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Nome do colaborador"/>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                ...
                <div>
                  ...
                  <input value={form.especificacao} onChange={e=>handleFormChange('especificacao',e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Ex: ATENDIMENTO"/>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={form.placa} onChange={e=>handleFormChange('placa',e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Ex: ABC-1234"/>
                </div>
                <div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button type="submit" disabled={saving} className="px-5 py-2 text-sm bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg disabled:opacity-60 transition">{saving?'Salvando...':'Salvar'}</button>
              </div>
            </form>
          </div>
        </div>
